...
ldatos = list_data(path,2015,1,1,2015,1,4)
##########
...
for data in ldatos[:20]:
    if data[7:9] == day:
        ...
        fcontent = read_data(path,data,elev=1,dist=298800,shape=1201)
        #print(type(fcontent))
        if type(fcontent) is tuple:
            ...
            vel[vel != 0] = 1
            ###
            dBZ_cor = preProcess(dBZ)
            Zc = dBZ_to_Zc(dBZ_cor,vel)
            Maccum = np.add(Maccum,Zc)
    else:
        #Plot
        print(Maccum.shape)
        fig = pl.figure(figsize=(10,8))
        plot_ppi(fig,Maccum,title="No",xlabel="Easting from radar (km)",ylabel="Northing from radar (km)",cmap="viridis")
        pl.show()
        Maccum = np.zeros((360,1201))
        contador = 0
        #Lee los datos, transforma y suma el resultado
        fcontent = read_data(path,data)
        if fcontent is tuple:
            ...
            vel = fcontent['data'][1]['sweep_data']['DB_VEL']['data']
            dBZ_cor = preProcess(dBZ)
            Zc = dBZ_to_Zc(dBZ_cor)
            Maccum = add_matrix(Maccum,Zc,contador)
